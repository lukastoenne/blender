...
void ViewLevelsNode::convertToOperations(NodeCompiler *compiler, const CompositorContext *context) const
{
	...
	if (input->isConnected()) {
		...
		/* calculate mean operation */
		{
			...
			compiler->mapInputSocket(input, operation->getInputSocket(0));
			compiler->mapOutputSocket(this->getOutputSocket(0), operation->getOutputSocket());
		}
		...
		/* calculate standard deviation operation */
		{
			...
			compiler->addOperation(operation);
			compiler->mapInputSocket(input, operation->getInputSocket(0));
			compiler->mapOutputSocket(this->getOutputSocket(1), operation->getOutputSocket());
		}
	}
	else {
		compiler->addOutputValue(getOutputSocket(0), 0.0f);
		compiler->addOutputValue(getOutputSocket(1), 0.0f);
	}
}
