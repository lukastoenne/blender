...
void SocketProxyNode::convertToOperations(NodeCompiler *compiler, const CompositorContext *context) const
{
	/* this should never be called, proxy nodes should be resolved */
	BLI_assert(false);
}
...
void SocketBufferNode::convertToOperations(NodeCompiler *compiler, const CompositorContext *context) const
{
	...
	compiler->addOperation(readOperation);
	
	compiler->mapInputSocket(input, writeOperation->getInputSocket(0));
	compiler->mapOutputSocket(output, readOperation->getOutputSocket());
}
