...
static void rna_StrandsModifier_seed_set(PointerRNA *ptr, int value)
{
	...
	/* Note: fibers array is regenerated by the modifier */
	if (smd->fibers && smd->seed != value) {
...
static void rna_StrandsModifier_num_fibers_set(PointerRNA *ptr, int value)
{
	StrandsModifierData *smd = (StrandsModifierData *)ptr->data;
	
	/* Note: fibers array is regenerated by the modifier */
	if (smd->fibers && smd->num_fibers != value) {
		MEM_freeN(smd->fibers);
		smd->fibers = NULL;
	}
...
static void rna_def_modifier_strands(BlenderRNA *brna)
{
	...
	RNA_def_property_int_sdna(prop, NULL, "seed");
	RNA_def_property_int_funcs(prop, NULL, "rna_StrandsModifier_seed_set", NULL);
	RNA_def_property_ui_text(prop, "Seed", "Seed value for root distribution");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
	prop = RNA_def_property(srna, "num_fibers", PROP_INT, PROP_UNSIGNED);
	RNA_def_property_int_sdna(prop, NULL, "num_fibers");
	RNA_def_property_int_funcs(prop, NULL, "rna_StrandsModifier_num_fibers_set", NULL);
	RNA_def_property_range(prop, 0, INT_MAX);
	RNA_def_property_ui_range(prop, 0, 1000000, 1, 3);
	...
	RNA_def_property_boolean_default(prop, true);
	RNA_def_property_ui_text(prop, "Show Strands", "Show control strand curves");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
	prop = RNA_def_property(srna, "show_fibers", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", MOD_STRANDS_SHOW_FIBERS);
	RNA_def_property_boolean_default(prop, true);
