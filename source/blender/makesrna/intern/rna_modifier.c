...
static void rna_def_modifier_strands(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "num_roots", PROP_INT, PROP_UNSIGNED);
	RNA_def_property_int_sdna(prop, NULL, "num_roots");
	RNA_def_property_int_funcs(prop, NULL, "rna_StrandsModifier_num_roots_set", NULL);
	RNA_def_property_range(prop, 0, INT_MAX);
	RNA_def_property_ui_range(prop, 0, 1000000, 1, 3);
	RNA_def_property_ui_text(prop, "Roots Number", "Number of rendered strand roots to generate");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
