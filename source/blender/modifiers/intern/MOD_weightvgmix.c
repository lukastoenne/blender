                            ...
                            struct DepsNodeHandle *node)
{
	...
	if (wmd->mask_tex_map_obj != NULL && wmd->mask_tex_mapping == MOD_DISP_MAP_OBJECT) {
		DEG_add_object_relation(node, wmd->mask_tex_map_obj, DEPSNODE_TYPE_TRANSFORM, "WeightVGMix Modifier");
	}
	if (wmd->mask_tex_mapping == MOD_DISP_MAP_GLOBAL) {
		DEG_add_object_relation(node, ob, DEPSNODE_TYPE_TRANSFORM, "WeightVGMix Modifier");
	}
}
