                            ...
                            struct DepsNodeHandle *node)
{
	...
	if (amd->start_cap != NULL) {
		DEG_add_object_relation(node, amd->start_cap, DEG_COMPONENT_TRANSFORM, "Hook Modifier Start Cap");
	}
	if (amd->end_cap != NULL) {
		DEG_add_object_relation(node, amd->end_cap, DEG_COMPONENT_TRANSFORM, "Hook Modifier End Cap");
	}
	if (amd->curve_ob) {
		DEG_add_object_relation(node, amd->end_cap, DEG_COMPONENT_GEOMETRY, "Hook Modifier Curve");
		DEG_add_special_eval_flag(scene->depsgraph, &amd->curve_ob->id, DAG_EVAL_NEED_CURVE_PATH);
	}
	if (amd->offset_ob != NULL) {
		DEG_add_object_relation(node, amd->offset_ob, DEG_COMPONENT_TRANSFORM, "Hook Modifier Offset");
	}
}
