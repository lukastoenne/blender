...
bvm_extern void MATRIX44_TO_LOC(float3 &loc, const matrix44 &m)
{
	copy_v3_v3(loc.data(), m.data[3]);
}
BVM_DECL_FUNCTION_VALUE(MATRIX44_TO_LOC)

bvm_extern void MATRIX44_TO_EULER(float3 &euler, int order, const matrix44 &m)
{
	mat4_to_eulO(euler.data(), (short)order, m.c_data());
}
BVM_DECL_FUNCTION_VALUE(MATRIX44_TO_EULER)

bvm_extern void MATRIX44_TO_AXISANGLE(float3 &axis, float &angle, const matrix44 &m)
{
	mat4_to_axis_angle(axis.data(), &angle, m.c_data());
}
BVM_DECL_FUNCTION_VALUE(MATRIX44_TO_AXISANGLE)

bvm_extern void MATRIX44_TO_SCALE(float3 &scale, const matrix44 &m)
{
	mat4_to_size(scale.data(), m.c_data());
}
BVM_DECL_FUNCTION_VALUE(MATRIX44_TO_SCALE)

bvm_extern void LOC_TO_MATRIX44(matrix44 &m, const float3 &loc)
{
	m = matrix44::identity();
...
bvm_extern void EULER_TO_MATRIX44(matrix44 &m, int order, const float3 &euler)
{
	m = matrix44::identity();
...
bvm_extern void AXISANGLE_TO_MATRIX44(matrix44 &m, const float3 &axis, float angle)
{
	m = matrix44::identity();
...
bvm_extern void SCALE_TO_MATRIX44(matrix44 &m, const float3 &scale)
{
	m = matrix44::identity();
...
bvm_extern void ADD_FLOAT(float &r, float a, float b)
{
	r = a + b;
}
...
bvm_extern void ARCTANGENT(float &r, float f)
{
	r = atanf(f);
}
BVM_DECL_FUNCTION_VALUE(ARCTANGENT)

bvm_extern void POWER(float &r, float a, float b)
{
	r = pow_safe(a, b);
}
BVM_DECL_FUNCTION_VALUE(POWER)

bvm_extern void LOGARITHM(float &r, float a, float b)
{
	r = log_safe(a, b);
}
BVM_DECL_FUNCTION_VALUE(LOGARITHM)

bvm_extern void MINIMUM(float &r, float a, float b)
{
	r = min_ff(a, b);
}
BVM_DECL_FUNCTION_VALUE(MINIMUM)

bvm_extern void MAXIMUM(float &r, float a, float b)
{
	r = max_ff(a, b);
}
BVM_DECL_FUNCTION_VALUE(MAXIMUM)

bvm_extern void ROUND(float &r, float f)
{
	r = floorf(f + 0.5f);
}
BVM_DECL_FUNCTION_VALUE(ROUND)

bvm_extern void LESS_THAN(float &r, float a, float b)
{
	r = (a < b)? 1.0f : 0.0f;
}
BVM_DECL_FUNCTION_VALUE(LESS_THAN)

bvm_extern void GREATER_THAN(float &r, float a, float b)
{
	r = (a > b)? 1.0f : 0.0f;
}
BVM_DECL_FUNCTION_VALUE(GREATER_THAN)

bvm_extern void MODULO(float &r, float a, float b)
{
	r = modulo_safe(a, b);
}
BVM_DECL_FUNCTION_VALUE(MODULO)

bvm_extern void ABSOLUTE(float &r, float f)
{
	r = fabs(f);
}
BVM_DECL_FUNCTION_VALUE(ABSOLUTE)

bvm_extern void CLAMP_ONE(float &r, float f)
{
	r = CLAMPIS(f, 0.0f, 1.0f);
}
BVM_DECL_FUNCTION_VALUE(CLAMP_ONE)

bvm_extern void SQRT(float &r, float f)
{
	r = sqrt_safe(f);
}
...
bvm_extern void SUB_FLOAT3(float3 &r, const float3 &a, const float3 &b)
{
	sub_v3_v3v3(r.data(), a.data(), b.data());
}
BVM_DECL_FUNCTION_VALUE(SUB_FLOAT3)

bvm_extern void MUL_FLOAT3(float3 &r, const float3 &a, const float3 &b)
{
...
bvm_extern void MUL_FLOAT3_FLOAT(float3 &r, const float3 &a, float b)
{
	mul_v3_v3fl(r.data(), a.data(), b);
}
BVM_DECL_FUNCTION_VALUE(MUL_FLOAT3_FLOAT)

bvm_extern void DIV_FLOAT3_FLOAT(float3 &r, const float3 &a, float b)
{
	r = float3(div_safe(a.x, b), div_safe(a.y, b), div_safe(a.z, b));
}
BVM_DECL_FUNCTION_VALUE(DIV_FLOAT3_FLOAT)

bvm_extern void AVERAGE_FLOAT3(float3 &r, const float3 &a, const float3 &b)
{
	r = float3(0.5f*(a.x+b.x), 0.5f*(a.y+b.y), 0.5f*(a.z+b.z));
}
BVM_DECL_FUNCTION_VALUE(AVERAGE_FLOAT3)

bvm_extern void DOT_FLOAT3(float &r, const float3 &a, const float3 &b)
{
	r = dot_v3v3(a.data(), b.data());
}
BVM_DECL_FUNCTION_VALUE(DOT_FLOAT3)

bvm_extern void CROSS_FLOAT3(float3 &r, const float3 &a, const float3 &b)
{
	r = float3(a.y*b.z - a.z*b.y, a.z*b.x - a.x*b.z, a.x*b.y - a.y*b.x);
}
BVM_DECL_FUNCTION_VALUE(CROSS_FLOAT3)

bvm_extern void NORMALIZE_FLOAT3(float3 &r_vec, float &r_len, const float3 &vec)
{
	r_len = normalize_v3_v3(r_vec.data(), vec.data());
}
BVM_DECL_FUNCTION_VALUE(NORMALIZE_FLOAT3)

bvm_extern void LENGTH_FLOAT3(float &len, const float3 &vec)
{
	len = len_v3(vec.data());
}
BVM_DECL_FUNCTION_VALUE(LENGTH_FLOAT3)

bvm_extern void ADD_MATRIX44(matrix44 &r, const matrix44 &a, const matrix44 &b)
{
	add_m4_m4m4(r.c_data(), a.c_data(), b.c_data());
}
BVM_DECL_FUNCTION_VALUE(ADD_MATRIX44)

bvm_extern void SUB_MATRIX44(matrix44 &r, const matrix44 &a, const matrix44 &b)
{
	sub_m4_m4m4(r.c_data(), a.c_data(), b.c_data());
}
BVM_DECL_FUNCTION_VALUE(SUB_MATRIX44)

bvm_extern void MUL_MATRIX44(matrix44 &r, const matrix44 &a, const matrix44 &b)
{
	mul_m4_m4m4(r.c_data(), a.c_data(), b.c_data());
}
BVM_DECL_FUNCTION_VALUE(MUL_MATRIX44)

bvm_extern void MUL_MATRIX44_FLOAT(matrix44 &r, const matrix44 &a, float b)
{
...
bvm_extern void DIV_MATRIX44_FLOAT(matrix44 &r, const matrix44 &a, float b)
{
	copy_m4_m4(r.c_data(), a.c_data());
...
bvm_extern void NEGATE_MATRIX44(matrix44 &r, const matrix44 &m)
{
...
bvm_extern void TRANSPOSE_MATRIX44(matrix44 &r, const matrix44 &m)
{
	transpose_m4_m4(r.c_data(), m.c_data());
}
BVM_DECL_FUNCTION_VALUE(TRANSPOSE_MATRIX44)

bvm_extern void INVERT_MATRIX44(matrix44 &r, const matrix44 &m)
{
	invert_m4_m4_safe(r.c_data(), m.c_data());
}
BVM_DECL_FUNCTION_VALUE(INVERT_MATRIX44)

bvm_extern void ADJOINT_MATRIX44(matrix44 &r, const matrix44 &m)
{
	adjoint_m4_m4(r.c_data(), m.c_data());
}
BVM_DECL_FUNCTION_VALUE(ADJOINT_MATRIX44)

bvm_extern void DETERMINANT_MATRIX44(float &r, const matrix44 &m)
{
	r = determinant_m4(m.c_data());
}
BVM_DECL_FUNCTION_VALUE(DETERMINANT_MATRIX44)

bvm_extern void MUL_MATRIX44_FLOAT3(float3 &r, const matrix44 &a, const float3 &b)
{
	mul_v3_m4v3(r.data(), a.c_data(), b.data());
}
BVM_DECL_FUNCTION_VALUE(MUL_MATRIX44_FLOAT3)

bvm_extern void MUL_MATRIX44_FLOAT4(float4 &r, const matrix44 &a, const float4 &b)
{
