...
static void time_draw_cache_ex(SpaceTime *stime, SpaceTimeCache **stc,
                               int sta, int end, char *cached_frames,
                               float col[4], float yoffs,
                               bool cache_baked, bool cache_outdated)
{
...
static void time_draw_cache_openvdb(SpaceTime *stime, Object *ob)
{
	if (!(stime->cache_display & (TIME_CACHE_DISPLAY|TIME_CACHE_SMOKE)) || (!ob))
		return;
	...
	if (!cache || cache->cached_frames == NULL) {
		return;
	}

	/* Same color as for pointcache */
	float col[4] = { 0.2f, 0.2f, 0.2f, 0.1f };

	SpaceTimeCache *stc = stime->caches.first;

	time_draw_cache_ex(stime, &stc,
	                   cache->startframe,
	                   cache->endframe,
	                   cache->cached_frames,
	                   col,
...
static void time_draw_cache(SpaceTime *stime, Object *ob, Scene *scene)
{
	...
	float col[4] = { 1.0f, 0.0f, 1.0f, 0.1f };
...
static void time_main_region_draw(const bContext *C, ARegion *ar)
{
	...
	time_draw_cache_openvdb(stime, obact);
